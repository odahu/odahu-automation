...
resource "local_file" "storage_class" {
  ...
  content = templatefile("${path.module}/templates/storage-class.tpl", {
    ...
    disk_encryption_set_id = azurerm_disk_encryption_set.this[0].id
  })
...
locals {
  aks_cluser_resource = var.kms_key_id == "" ? azurerm_kubernetes_cluster.this[0] : azurerm_kubernetes_cluster.encrypted[0]
...
resource "azurerm_kubernetes_cluster" "encrypted" {
  count = var.kms_key_id == "" ? 0 : 1
...
resource "azurerm_kubernetes_cluster_node_pool" "aks" {
  ...
  enable_node_public_ip = false
  kubernetes_cluster_id = local.aks_cluser_resource.id
...
resource "null_resource" "bastion_kubeconfig" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "mkdir -p ~/.kube && printf \"${local.aks_cluser_resource.kube_config_raw}\" > ~/.kube/config"
    ]
  }

  depends_on = [
    azurerm_kubernetes_cluster.this,
    azurerm_kubernetes_cluster.encrypted
  ]
}
...
resource "null_resource" "kube_api_check" {
  ...
  provisioner "local-exec" {
    command = "timeout 1200 bash -c 'until curl -sk https://${local.aks_cluser_resource.fqdn}; do sleep 20; done'"
  }
